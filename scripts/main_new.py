from fastapi import FastAPI, File, UploadFile, HTTPException, BackgroundTasks, Body
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import JSONResponse
from fastapi.responses import FileResponse
from fastapi.requests import Request
from fastapi.exceptions import RequestValidationError
from pydantic import BaseModel
from typing import List, Optional
import os
import uuid
import shutil
from pathlib import Path
import cv2
import numpy as np
from moviepy.editor import VideoFileClip
import json
from datetime import datetime
from typing import Dict, Any
import asyncio
import base64
from starlette.exceptions import HTTPException as StarletteHTTPException
# from video_cropper_oop import cropper, VideoCropper, CropperConfig  # 실제 구현시 주석 해제

# Pydantic 모델 정의
class BoundingBox(BaseModel):
    x: int
    y: int
    width: int
    height: int

class ProcessingConfig(BaseModel):
    job_id: str
    bounding_box: BoundingBox
    additional_params: Optional[Dict] = {}

app = FastAPI(title="CruxCut Video Processing API", version="1.0.0")

@app.exception_handler(StarletteHTTPException)
async def custom_http_exception_handler(request: Request, exc: StarletteHTTPException):
    return JSONResponse(
        status_code=exc.status_code,
        content={"error": exc.detail}
    )

@app.exception_handler(RequestValidationError)
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    return JSONResponse(
        status_code=422,
        content={
            "error": "Validation Error",
            "details": exc.errors(),
            "body": exc.body
        }
    )
    
# CORS 설정
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Next.js 개발 서버
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# 디렉토리 설정
UPLOAD_DIR = Path("uploads")
PROCESSED_DIR = Path("processed")
TEMP_DIR = Path("temp")
FRAMES_DIR = Path("frames")  # 프레임 저장 디렉토리 추가

# 디렉토리 생성
for directory in [UPLOAD_DIR, PROCESSED_DIR, TEMP_DIR, FRAMES_DIR]:
    directory.mkdir(exist_ok=True)

# 작업 상태 저장소 (실제 환경에서는 Redis나 데이터베이스 사용)
job_status: Dict[str, Dict[str, Any]] = {}

class VideoProcessor:
    """비디오 처리 클래스"""
    
    @staticmethod
    def extract_first_frame(video_path: str, output_path: str) -> bool:
        """비디오에서 첫 번째 프레임 추출"""
        try:
            cap = cv2.VideoCapture(video_path)
            ret, frame = cap.read()
            cap.release()
            
            if ret:
                cv2.imwrite(output_path, frame)
                return True
            return False
        except Exception as e:
            print(f"Frame extraction error: {e}")
            return False
    
    @staticmethod
    def frame_to_base64(image_path: str) -> str:
        """이미지 파일을 base64로 인코딩"""
        try:
            with open(image_path, "rb") as image_file:
                encoded_string = base64.b64encode(image_file.read()).decode()
                return f"data:image/jpeg;base64,{encoded_string}"
        except Exception as e:
            print(f"Base64 encoding error: {e}")
            return ""
    
    @staticmethod
    def detect_climber(frame):
        """클라이머 감지 (간단한 예시 - 실제로는 더 복잡한 AI 모델 사용)"""
        # OpenCV를 사용한 간단한 움직임 감지
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        # 실제 구현에서는 YOLO, MediaPipe 등의 AI 모델 사용
        return (100, 100, 200, 200)  # x, y, width, height
    
    @staticmethod
    def crop_and_track_video_with_bbox(input_path: str, output_path: str, job_id: str, bbox: BoundingBox):
        """사용자 지정 bounding box를 사용한 비디오 크롭 및 추적 처리"""
        try:
            # 작업 상태 업데이트
            job_status[job_id]["status"] = "processing"
            job_status[job_id]["progress"] = 10
            
            # 비디오 로드
            cap = cv2.VideoCapture(input_path)
            fps = int(cap.get(cv2.CAP_PROP_FPS))
            total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
            
            # 원본 비디오 크기 정보
            original_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
            original_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
            
            # 출력 비디오 설정 (크롭된 영역 크기로 설정)
            crop_width = bbox.width
            crop_height = bbox.height
            
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            out = cv2.VideoWriter(output_path, fourcc, fps, (crop_width, crop_height))
            
            frame_count = 0
            
            while True:
                ret, frame = cap.read()
                if not ret:
                    break
                
                # 사용자 지정 bounding box로 크롭
                x, y, w, h = bbox.x, bbox.y, bbox.width, bbox.height
                
                # 경계 검사
                x = max(0, min(x, original_width - w))
                y = max(0, min(y, original_height - h))
                w = min(w, original_width - x)
                h = min(h, original_height - y)
                
                # 크롭된 프레임 생성
                cropped_frame = frame[y:y+h, x:x+w]
                
                # 프레임 저장
                out.write(cropped_frame)
                
                # 진행률 업데이트
                frame_count += 1
                progress = int((frame_count / total_frames) * 80) + 10
                job_status[job_id]["progress"] = min(progress, 90)
                
            cap.release()
            out.release()
            
            # 작업 완료
            job_status[job_id]["status"] = "completed"
            job_status[job_id]["progress"] = 100
            job_status[job_id]["output_file"] = output_path
            
        except Exception as e:
            job_status[job_id]["status"] = "failed"
            job_status[job_id]["error"] = str(e)

@app.post("/upload")
async def upload_video(file: UploadFile = File(...)):
    """비디오 파일 업로드 및 첫 프레임 추출"""
    
    # 파일 형식 검증
    if not file.content_type.startswith('video/'):
        raise HTTPException(status_code=400, detail="Only video files are allowed")
    
    # 고유 작업 ID 생성
    job_id = str(uuid.uuid4())[:8]
    
    # 파일 저장
    file_extension = Path(file.filename).suffix
    input_filename = f"{job_id}_input{file_extension}"
    input_path = UPLOAD_DIR / input_filename
    
    with open(input_path, "wb") as buffer:
        shutil.copyfileobj(file.file, buffer)
    
    # 첫 프레임 추출
    frame_filename = f"{job_id}_first_frame.jpg"
    frame_path = FRAMES_DIR / frame_filename
    
    if not VideoProcessor.extract_first_frame(str(input_path), str(frame_path)):
        raise HTTPException(status_code=500, detail="Failed to extract first frame")
    
    # 첫 프레임을 base64로 인코딩
    frame_base64 = VideoProcessor.frame_to_base64(str(frame_path))
    if not frame_base64:
        raise HTTPException(status_code=500, detail="Failed to encode frame")
    
    # 비디오 정보 추출
    cap = cv2.VideoCapture(str(input_path))
    video_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    video_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    fps = int(cap.get(cv2.CAP_PROP_FPS))
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    duration = total_frames / fps if fps > 0 else 0
    cap.release()
    
    # 작업 상태 초기화
    job_status[job_id] = {
        "status": "uploaded",
        "progress": 0,
        "created_at": datetime.now().isoformat(),
        "input_file": str(input_path),
        "frame_file": str(frame_path),
        "video_info": {
            "width": video_width,
            "height": video_height,
            "fps": fps,
            "duration": duration,
            "total_frames": total_frames
        }
    }
    
    return {
        "job_id": job_id,
        "message": "Video uploaded successfully",
        "first_frame": frame_base64,
        "video_info": {
            "width": video_width,
            "height": video_height,
            "fps": fps,
            "duration": duration,
            "total_frames": total_frames
        }
    }

@app.post("/process")
async def process_video(background_tasks: BackgroundTasks, config: ProcessingConfig):
    """사용자가 설정한 bounding box로 비디오 처리 시작"""
    
    # 작업 ID 검증
    if config.job_id not in job_status:
        raise HTTPException(status_code=404, detail="Job ID not found")
    
    job = job_status[config.job_id]
    
    if job["status"] != "uploaded":
        raise HTTPException(status_code=400, detail="Video not ready for processing")
    
    # 출력 파일 경로
    output_filename = f"{config.job_id}_processed.mp4"
    output_path = PROCESSED_DIR / output_filename
    
    # 작업 상태 업데이트
    job_status[config.job_id]["status"] = "processing"
    job_status[config.job_id]["progress"] = 5
    job_status[config.job_id]["output_file"] = str(output_path)
    job_status[config.job_id]["bounding_box"] = config.bounding_box.dict()
    
    # 백그라운드에서 비디오 처리 시작
    background_tasks.add_task(
        VideoProcessor.crop_and_track_video_with_bbox,
        job["input_file"],
        str(output_path),
        config.job_id,
        config.bounding_box
    )
    
    return {
        "job_id": config.job_id,
        "message": "Video processing started with custom bounding box",
        "bounding_box": config.bounding_box.dict()
    }

@app.get("/frame/{job_id}")
async def get_first_frame(job_id: str):
    """첫 프레임 이미지 파일로 반환"""
    if job_id not in job_status:
        raise HTTPException(status_code=404, detail="Job ID not found")
    
    job = job_status[job_id]
    frame_path = job.get("frame_file")
    
    if not frame_path or not os.path.exists(frame_path):
        raise HTTPException(status_code=404, detail="Frame file not found")
    
    return FileResponse(
        path=frame_path,
        media_type='image/jpeg',
        filename=f"first_frame_{job_id}.jpg"
    )

@app.get("/status/{job_id}")
async def get_job_status(job_id: str):
    """작업 상태 조회"""
    if job_id not in job_status:
        return JSONResponse(
            status_code=404,
            content={"error": f"Job ID '{job_id}' not found"}
        )
    return JSONResponse(content=job_status[job_id])

@app.get("/download/{job_id}")
async def download_processed_video(job_id: str):
    """처리된 비디오 다운로드"""
    if job_id not in job_status:
        raise HTTPException(status_code=404, detail="Job ID not found")
    
    job = job_status[job_id]
    
    if job["status"] != "completed":
        raise HTTPException(status_code=400, detail="Job not completed yet")
    
    output_path = job["output_file"]
    
    if not os.path.exists(output_path):
        raise HTTPException(status_code=404, detail="Processed file not found")
    
    return FileResponse(
        path=output_path,
        media_type='video/mp4',
        filename=f"cruxcut_processed_{job_id}.mp4"
    )

@app.delete("/cleanup/{job_id}")
async def cleanup_job(job_id: str):
    """작업 관련 파일들 정리"""
    if job_id not in job_status:
        raise HTTPException(status_code=404, detail="Job ID not found")
    
    job = job_status[job_id]
    files_to_delete = []
    
    # 관련 파일들 수집
    if "input_file" in job and os.path.exists(job["input_file"]):
        files_to_delete.append(job["input_file"])
    
    if "frame_file" in job and os.path.exists(job["frame_file"]):
        files_to_delete.append(job["frame_file"])
    
    if "output_file" in job and os.path.exists(job["output_file"]):
        files_to_delete.append(job["output_file"])
    
    # 파일 삭제
    deleted_files = []
    for file_path in files_to_delete:
        try:
            os.remove(file_path)
            deleted_files.append(file_path)
        except Exception as e:
            print(f"Failed to delete {file_path}: {e}")
    
    # 작업 상태에서 제거
    del job_status[job_id]
    
    return {
        "message": f"Cleanup completed for job {job_id}",
        "deleted_files": deleted_files
    }

@app.get("/")
async def root():
    return {"message": "CruxCut Video Processing API", "version": "1.0.0"}

if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=8000)